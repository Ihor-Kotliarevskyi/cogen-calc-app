...
export default function SavedScenariosScreen() {
  ...
  const handleSave = () => {
    if (!name.trim()) return;
    saveScenario(name.trim(), P, result);
    setName('');
  ...
  const handlePrintCurrent = () => {
    setShowPrint({
      ...
      P,
      metrics: {
        ecg: result.ecg, ep: result.ep, pb: result.pb,
        tot: result.tot, net: result.net, gcT: result.gcT,
      },
    });
    setTimeout(() => {
      window.print();
    ...
    <div className="screen active">
      ...
      {scenarios.length > 0 && (
        <div className="export-bar">
          <button className="btn-export" onClick={() => exportToJSON(scenarios)}>📄 JSON</button>
          <button className="btn-export" onClick={() => exportToCSV(scenarios)}>📊 CSV</button>
      ...
      {scenarios.length === 0 ? (
        <div className="empty-state">
          ...
          <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 4 }}>
            Натисніть «+ Зберегти» щоб зберегти поточні параметри
          </div>
        </div>
      ) : (
        scenarios.map((s) => {
            ...
            <div key={s.id} className="card sc-saved-card">
              ...
              <div className="sc-saved-metrics">
                <div className="sc-saved-metric">
                  <span className="sc-saved-metric-label">Собів.</span>
                  <span style={{ color: s.metrics.ecg < s.metrics.ep ? 'var(--green)' : 'var(--red)' }}>
                    {fG(s.metrics.ecg)}
                  </span>
                </div>
                <div className="sc-saved-metric">
                  <span className="sc-saved-metric-label">Окупн.</span>
                  <span style={{ color: pbColor }}>
                    {s.metrics.pb ? s.metrics.pb.toFixed(1) + ' р.' : '∞'}
                  </span>
                </div>
                <div className="sc-saved-metric">
                  <span className="sc-saved-metric-label">Прибуток</span>
                  <span style={{ color: netColor }}>{fM(s.metrics.net)}</span>
                </div>
              </div>
            </div>
          );
