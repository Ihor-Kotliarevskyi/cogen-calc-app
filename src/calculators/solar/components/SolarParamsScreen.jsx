...
const SLIDERS = [
  ...
  { key: 'reserveShare', label: 'Частка генерації в резервуванні', min: 0, max: 1, step: 0.05, fmt: (v) => `${(v * 100).toFixed(0)}%` },
        ...
        <div className="card">
          {SLIDERS.map((slider) => (
            <NumericSliderField
              ...
              slider={slider}
              value={P[slider.key]}
              onChange={(key, value) => dispatch({ type: 'SET_PARAM', key, value })}
            />
          ))}
        </div>
      </div>
